...
# MAGIC %md # Remote Model Registry example
# MAGIC 
...
model_names = [m.name for m in client.search_registered_models(filter_string=m.name.startswith(prefix))]
print(model_names)
...
client.transition_model_version_stage(model3_name, 2, 'Staging')
client.get_model_version(model3_name, 2)

# COMMAND ----------

model1_version = client.get_model_version(model1_name, 2)  # for cleaning up files later
client.delete_registered_model(model1_name)
assert model1_name not in [m.name for m in client.search_registered_models()]
